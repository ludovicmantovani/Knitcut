...
public class Music_Scene : MonoBehaviour
{
	...
	/// </summary>
	/// <returns>Master volume of MUSIC.</returns>
	public float volume
	{
		get
		{
	...
	public virtual void StartAutopilot(int numInitTracks, float fadeInDuration = 3f, bool ignoreTimeScale = true)
	{
		...
		{
			Debug.LogError("MUSIC: Number of active autopilot tracks must be positive.");
			return;
		}
		...
		if (verbose)
		{
			Debug.Log("MUSIC: Starting autopilot.");
		}
		...
		Music.Utils.Shuffle(tracksRandomized);
		if (_playing.Count == 0)
		{
			...
			if (verbose)
			{
				Debug.Log(string.Format("MUSIC: Nothing is playing. Starting {0} layers.", maxIndex));
			}
	...
	public virtual void FadeInLayer(AudioSource layer, float duration = 1f, bool ignoreTimeScale = true)
	{
		if (verbose)
		{
			Debug.Log(string.Format("MUSIC: Fading in {0}.", layer.clip.name));
		}
	...
	public virtual void FadeOutLayer(AudioSource layer, float duration = 1f, bool ignoreTimeScale = true)
	{
		if (verbose)
		{
			Debug.Log(string.Format("MUSIC: Fading out {0}.", layer.clip.name));
		}
    ...
    void LoadVolumeSystem()
    {
        ...
        _volume = data.volume / volumeDivider;
    }

    public void UpdateVolume(float newVolume)
    {
		volume = newVolume / volumeDivider;
    }
	...
	private IEnumerator AutopilotCoroutine()
	{
		...
		while (_isAutopilotActive && changeTracksAutomatically)
		{
			...
			float absolute = numActiveTracks + Music.Utils.NormalDistributionRandom(-trackNumberVariance, trackNumberVariance);
			bool isFadeIn = _playing.Count <= 1 || absolute > _playing.Count;

			if (verbose)
			{
				Debug.Log(string.Format("MUSIC: Playing: {0}, idle: {1}, action is fade in: {2}.", _playing.Count,
			...
			{  // Fade in an idle track.
			   // Start one of the first tracks in queue.
				AudioSource layer = Music.Utils.GetOneOfTheFirst(_idle);
				if (layer != null)
				{
			...
			{  // Fade out a playing track.
				AudioSource layer = Music.Utils.GetOneOfTheFirst(_playing);
				if (layer != null)
				{
